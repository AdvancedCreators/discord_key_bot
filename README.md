...
| コマンド | 説明 |
|---------|------|
| `/reminder_daily hour:<時刻>` | 返却催促リマインドの時刻を変更（0 で停止。未返却のまま 30 分経過後に送信） |
| `/reminder_idle hours:<時間>` | 場所未報告リマインドの時間を変更（0 で停止） |

> ⚠️ 持ち主が変わると、リマインド設定は自動でデフォルト（21 時 / 2 時間）に戻ります。
...
| 種類 | デフォルト | 動作 |
|------|-----------|------|
| **返却催促リマインド (daily)** | 21 時以降 | 未返却（閉めた/持ち出し中）のまま 30 分以上経過していると「鍵を返し忘れていませんか？」を 1 日 1 回送信 |
| **場所未報告リマインド (idle)** | 2 時間 | 鍵を閉めて場所未報告のまま指定時間経過で「場所の報告を忘れていませんか？」を送信 |
...
返却催促リマインドだけ最終日に通知することで「返却日に忘れない」状態を作ります。
